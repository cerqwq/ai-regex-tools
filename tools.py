...
class AIRegexTools:
    ...
    def generate_regex(self, description: str, examples: List[str] = None) -> Dict:
        """生成正则表达式"""
        if not self.client:
...
3. 匹配示例"""
    ...
    def test_regex(self, regex: str, test_strings: List[str]) -> Dict:
        """测试正则表达式"""
    ...
    def optimize_regex(self, regex: str) -> Dict:
        """优化正则表达式"""
        if not self.client:
            return {"error": "LLM客户端未配置"}
...
{{
    ...
    def extract_patterns(self, text: str, pattern_type: str) -> List[str]:
        """提取文本中的模式"""
        patterns = {
    ...
    def generate_from_examples(self, positive: List[str], negative: List[str] = None) -> Dict:
        """从示例生成正则"""
        if not self.client:
            return {"error": "LLM客户端未配置"}
...
def create_tools(**kwargs) -> AIRegexTools:
    """创建正则工具"""
    return AIRegexTools(**kwargs)


if __name__ == "__main__":
    tools = create_tools()

    print("AI Regex Tools")
    print()

    # 测试
    result = tools.generate_regex("匹配中国手机号码", ["13812345678", "15900001111"])
